...
local function trim_trailing_whitespace()
  if vim.bo.buftype ~= "" or CONFIG.trim_whitespace_skip_ft[vim.bo.filetype] then return end
...
autocmd("LspAttach", {
  ...
  callback = function(args)
    if vim.b[args.buf]._inlay_hints_auto_enabled then return end
...
autocmd("BufWinEnter", {
  ...
  callback = function()
    ...
    if bt == "terminal" or bt == "nofile" or bt == "acwrite" then return end
...
-- Switches to absolute line numbers in insert mode
autocmd("InsertEnter", {
  group = augroup,
  callback = function()
...
-- Switches back to relative line numbers in normal mode
autocmd("InsertLeave", {
  group = augroup,
  callback = function()
    vim.wo.number = true
...
autocmd("WinEnter", {
  group = augroup,
  callback = function()
    if vim.bo.buftype ~= "terminal" then vim.wo.cursorline = true end
  end,
})
autocmd("WinLeave", {
  group = augroup,
  callback = function()
    if vim.bo.buftype ~= "terminal" then vim.wo.cursorline = false end
  end,
})
