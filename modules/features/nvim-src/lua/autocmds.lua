...
-- Automatically saves Neovim session (Session.vim) for project directories
local session_group = vim.api.nvim_create_augroup("UserSession", { clear = true })
local function save_session()
	local cwd = vim.fn.getcwd()
	local home = vim.fn.expand("~")

	-- Skip session creation in home directory directly, /tmp, or special locations
	if cwd == home or cwd:find("^/tmp") or cwd:find("^/private/tmp") then
		return
	end

	-- Skip if current buffer is temporary/special (git, help, oil, terminal, etc.)
	local ft = vim.bo.filetype
	if ft == "gitcommit" or ft == "gitrebase" or ft == "oil" or ft == "starter" then
		return
	end

	-- Ensure we have at least one real file buffer before saving session
	local has_real_buffer = false
	for _, buf in ipairs(vim.api.nvim_list_bufs()) do
		if vim.api.nvim_buf_is_loaded(buf) and vim.bo[buf].buftype == "" then
			local name = vim.api.nvim_buf_get_name(buf)
			if name ~= "" and not name:find("^term://") then
	...
	if not has_real_buffer then
		if vim.fn.filereadable("Session.vim") == 1 then
			pcall(vim.fn.delete, "Session.vim")
		end
		return
	end
...
autocmd({ "VimLeavePre", "BufWritePost", "FocusLost" }, {
	group = session_group,
