...
require("lualine").setup({
	...
	sections = {
		...
		lualine_b = {
			...
			{ "diagnostics", sources = { "nvim_diagnostic" }, symbols = { error = "×", warn = "▲", info = "•", hint = "•" }, padding = { left = 1, right = 0 } },
		},
		lualine_c = { { "filename", path = 1, symbols = { modified = " ", readonly = " ", new = " ", unnamed = "[No Name]" } } },
		lualine_x = {
			{ function()
			...
			end, icon = { "  ", align = "left" }, color = { fg = c.comment },
				cond = function() return #vim.lsp.get_clients({ bufnr = 0 }) > 0 and vim.bo.filetype ~= "" end },
		},
		lualine_y = { { "filetype", colored = false, padding = { left = 1, right = 1 } } },
		lualine_z = { { function() return vim.fn.line(".") .. "/" .. vim.fn.line("$") end, padding = { left = 1, right = 1 } } },
	},
	extensions = {},
})
...
local function statuscolumn()
	...
	if foldcolumn > 0 then
		...
		if foldclosed > 0 then foldcol = "▸"
		elseif foldlevel > 0 then foldcol = "▾"
		else foldcol = " " end
		foldcol = foldcol .. " "
	end
	...
	if vim.v.relnum == 0 then numcol = string.format("%" .. width .. "d ", lnum)
	else numcol = string.format("%" .. width .. "d ", vim.v.relnum) end
