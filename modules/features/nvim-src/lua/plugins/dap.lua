...
dap.configurations.java = {
	...
	{ type = "java", request = "attach", name = "Attach to custom host/port", hostName = function() local host = vim.fn.input("Host: ", "127.0.0.1") return host ~= "" and host or "127.0.0.1" end, port = function() local port = tonumber(vim.fn.input("Port: ", "5005")) return port or 5005 end },
}
...
if lldb then
	...
	local lldb_configs = {
		{ type = "lldb", request = "launch", name = "Launch executable", program = function() local cwd = vim.fn.getcwd() local default = cwd .. "/target/debug/" .. vim.fn.fnamemodify(cwd, ":t") return vim.fn.input("Path to executable: ", default, "file") end, cwd = "${workspaceFolder}", stopOnEntry = false, args = function() return prompt_args("Args: ") end },
...
map("n", "<leader>mO", dap.step_out, { desc = "Step out" })
