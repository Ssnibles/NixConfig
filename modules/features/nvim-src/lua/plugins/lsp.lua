...
local function smart_rename_replace()
  ...
  if has_word then
    choices[#choices + 1] = {
      ...
      action = function() open_grug_far({ search = current_name, flags = "--fixed-strings --word-regexp" }) end,
    }
    choices[#choices + 1] = {
      label = "Project replace regex",
      action = function() open_grug_far({ search = ("\\b%s\\b"):format(escape_rg_regex(current_name)) }) end,
    }
  end
