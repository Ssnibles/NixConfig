			...
			vim.api.nvim_buf_set_extmark = function(bufnr, ns_id, line, col, opts)
				if ns_id == ns then
					...
					if line_text then
						...
						if col > max_col then col = max_col elseif col < 0 then col = 0 end
					end
				end
...
local function detect_flake_root()
	...
	if from_env and from_env ~= "" then
		...
		if vim.uv.fs_stat(root .. "/flake.nix") then return root end
	end

	local cwd = vim.uv.cwd() or vim.fn.getcwd()
	local flake = vim.fs.find("flake.nix", { path = cwd, upward = true })[1]
	if flake then return vim.fs.dirname(flake) end

	local fallback = (vim.env.HOME or "~") .. "/NixConfig"
	if vim.uv.fs_stat(fallback .. "/flake.nix") then return fallback end

	return nil
end

local function host_exists(flake_root, host)
	if not flake_root or host == "" then return false end
...
local function detect_nix_host(flake_root)
	...
	if from_env and from_env ~= "" then
		if not flake_root or host_exists(flake_root, from_env) then return from_env end
		vim.schedule(function()
			vim.notify(("NIX_CONFIG_HOST=%s does not match flake hosts; falling back"):format(from_env), vim.log.levels.WARN)
		end)
	end

	local hostname = vim.uv.os_gethostname() or ""
	local short_hostname = hostname:match("^[^.]+") or hostname
	if short_hostname ~= "" and host_exists(flake_root, short_hostname) then return short_hostname end

	if host_exists(flake_root, "desktop") then return "desktop" end
	if host_exists(flake_root, "laptop") then return "laptop" end
...
local function prompt_lsp_rename()
	...
	if current_name == "" then lsp.buf.rename() return end

	vim.ui.input({ prompt = "Rename symbol to: ", default = current_name }, function(new_name)
		if new_name == nil or new_name == "" or new_name == current_name then return end
...
local function open_grug_far(prefills)
	...
	if not ok then vim.notify("grug-far.nvim is not available", vim.log.levels.ERROR) return end
...
local function open_literal_project_replace()
	local current_name = vim.fn.expand("<cword>")
	if current_name == "" then vim.notify("No word under cursor", vim.log.levels.WARN) return end
...
local function smart_rename_replace()
	...
	if has_word then
		choices[#choices + 1] = { label = "Project replace word (literal)", action = open_literal_project_replace }
		choices[#choices + 1] = { label = "Project replace regex", action = open_regex_project_replace }
	end
	...
	if #choices == 1 then choices[1].action() return end
...
in
  pkgs // { unstable = unstable; }
	...
	nixd_options = {
		...
		["home-manager"] = {
			expr = ('(builtins.getFlake %s).nixosConfigurations.%s.options."home-manager".users.type.getSubOptions []'):format(flake_ref, host_attr),
		},
	}
end

local function attach_lsp_keymaps(bufnr)
	if vim.b[bufnr]._lsp_keymaps_attached then return end
	...
	map("gr", function() require("fzf-lua").lsp_references({ jump_to_single_result = true }) end, "Find references")
	...
	map("<leader>ca", function() lsp.buf.code_action() end, "Code action")
end

lsp.config("*", {
	capabilities = capabilities,
	flags = { debounce_text_changes = 150 },
	on_attach = function(_, bufnr) attach_lsp_keymaps(bufnr) end,
})
...
local ts_js_inlay_hints = {
	parameterNames = { enabled = "literals" }, parameterTypes = { enabled = true },
	variableTypes = { enabled = true }, propertyDeclarationTypes = { enabled = true },
	functionLikeReturnTypes = { enabled = true }, enumMemberValues = { enabled = true },
}
...
vim.api.nvim_create_user_command("LspHealth", function()
	...
	for _, server in ipairs(managed_servers) do
		...
		if exe ~= "" then lines[#lines + 1] = ("- %s: OK (%s)"):format(server.name, exe)
		else lines[#lines + 1] = ("- %s: missing `%s`"):format(server.name, server.cmd) end
	end

	local active = vim.lsp.get_clients({ bufnr = 0 })
	if #active > 0 then
		local names = {}
		for _, client in ipairs(active) do names[#names + 1] = client.name end
