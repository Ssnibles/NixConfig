...
ls.add_snippets("typst", {
	s({ trig = "page", desc = "Mathematical textbook style page boilerplate" }, {
		t({ "#set page(", "" }),
		t({ '  paper: "a4",', "" }),
		...
		t({ "    if page_num > 1 [", "" }),
		t({ '      #set text(size: 8.5pt, font: "New Computer Modern")', "" }),
		t({ "      #grid(", "" }),
		...
		t({ '      #set text(size: 9pt, font: "New Computer Modern")', "" }),
		t({ "      #align(center)[#str(page_num)]", "" }),
		t({ "    ]", "" }),
		t({ "  },", "" }),
		t({ ")", "", "" }),
		t({ '#set text(font: "New Computer Modern", size: 10pt, lang: "en")', "" }),
		...
		t({ '  #text(size: 22pt, weight: "regular", style: "italic")[#it.body]', "" }),
		t({ "  #v(0.4em)", "" }),
		t({ '  #text(size: 11pt, weight: "regular")[#smallcaps[Chapter 1]]', "" }),
		t({ "]", "", "" }),
		t({ "// Level 2: Bold small-caps section title with trailing line", "" }),
		t({ "#show heading.where(level: 2): it => block(above: 1.8em, below: 1.2em)[", "" }),
		t({ "  #box(baseline: 0%, [", "" }),
		t({ '    #text(weight: "bold")[#smallcaps[#it.body]]', "" }),
		t({ "    #h(0.8em)", "" }),
		t({ "    #box(line(length: 6cm, stroke: 0.8pt + black))", "" }),
		t({ "  ])", "" }),
		t({ "]", "", "" }),
		t({ "= " }),
		i(1, "Mathematical Language"),
		t({ "", "", "== " }),
		i(2, "1.1. Sets"),
		t({ "", "", "" }),
		i(3, "Content here..."),
	...
	s({ trig = "truthtable", desc = "Formatted digital logic truth table" }, {
		...
		t({ '  fill: (x, y) => if y <= 1 { rgb("efefef") } else { none },', "", "" }),
